...
function UserBio(props) {
    ...
    const handleSave = () => {
        ...
        apiCall("/api/user/" + props?.currentUser?.id, "PUT", userDTO).then(() => {
            props.getStudent();
    ...
    return (
        <div style={{backgroundColor: "lightgoldenrodyellow"}}>
            <div className={"container d-flex p-4 justify-content-between align-items-center"}>
                <div className={"d-flex align-items-center justify-content-center"}>
                    <div><AiOutlineUser size={"70px"}/></div>
                    <div>
                ...
                <div>
                    <p className={"display-6"}>{props?.currentUser?.bio ? props?.currentUser?.bio : "No bio yet"}</p>
                </div>
                <div>
                    <button onClick={() => setModalVisible(true)} className={"btn btn-warning"}>Edit Profile</button>
                </div>
            </div>
            <Rodal visible={modalVisible} onClose={() => setModalVisible(false)} height={500}>
                <div>
                    <div className="mb-3">
