...
function VideoCourse(props) {
    ...
    return (
        <div>
            <div className={"d-flex justify-content-center align-items-center gap-4 pb-4"}>
                <h5>My Progress</h5>
                <Space wrap>
                    {
                        progressPercentage === 100 ? <Progress type="circle" percent={100} format={() => 'Done'}/> :
                            <Progress
                                type="circle"
            ...
            {
                videoObj && <div className="bg-info text-center shadow-lg rounded p-3 w-25 my-4">
               <span style={{
                   color: 'white',
                   fontSize: '18px',
                        ...
                        <div className={'d-flex flex-column'}>
                            {videoObj?.courseVideo.map((item) => (
                                <div key={item.id} className={"d-flex align-items-center gap-2"}>
                                    {
                                        item.isFinished ?
                                            <div className={"checkBox "}><ImCheckboxChecked color={"green"} size={20}/>
                                            </div> :
                                            <div className={"checkBox "}><ImCheckboxUnchecked color={"black"}
                                                                                              size={20}/></div>
                                    }
                                    ...
                                    <div key={item.id}
                                         ...
                                         className={'courseName border'} onClick={() => showVideo(item)}>
