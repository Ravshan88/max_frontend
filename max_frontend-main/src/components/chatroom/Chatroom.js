...
function Chatroom(props) {
    ...
    function getUsers() {
        apiCall("/api/user", "GET", null, {myId: props?.currentUser?.id}).then(({data}) => {
            setUsers(data);
        });
    }

    function showMessage(id) {
        apiCall(`/api/message?senderId=${props?.currentUser?.id}&receiverId=${id}`, "GET").then((res) => {
                        ...
                        {!messages.length && receiverId === "" ? (
                            ...
                            <div className='ota-chat pb-5 position-relative'>
                                        ...
                                        messages.length === 0 ? <div>
                                            ...
                                            messages?.map((item) => (
                                                <div
                                                    key={item?.id}
                                                    className={`message ${
                                                        item.sender.id === props.currentUser?.id ? " right" : " left"
                                                    }`}
                                                >
                                                    <p className='text-message'>{item.message}</p>
                                                    <small
                                                        className={`message ${
                                                            item.sender.id === props.currentUser?.id ? " rightTime" : " leftTime"
                                                        }`}
                                                    >
                                                        {
                                ...
                                <div className='inputSenderdiv'>
                                    ...
                                    />
                                    <button onClick={sendMessage} className='btn btn-info'>send
                                    </button>
                                </div>
                            </div>
                        )}
