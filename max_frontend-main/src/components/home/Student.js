...
function Student(props) {
    ...
    function getUserCourse() {
        apiCall("/api/course/video", "GET", null, {userId: props?.currentUser?.id}).then((res) => {
    ...
    function deleteCourse(item) {
        apiCall("/api/course", "DELETE", null, {userId: item.userId, courseId: item.id}).then((res) => {
                    ...
                    >
                        {
                            courseVideo.map((item, index) => (
                                <div key={item.id} className={" d-flex justify-content-between btn-group"}>
                                    <div
                                        onClick={() => showVideo(item)}
                                        className={"link-settings"}

                                        style={{
                                            background: item.id === videoObj?.id ? "#2b76bd" : "#7ec8f2",
                                            cursor: 'pointer'
                                        }}
                                    >
                                        <div>
                                    ...
                                    <div onClick={() => deleteCourse(item)} className={"deleteCourseBtn"}>
                                        <DeleteFilled/>
                                    </div>
                                </div>
